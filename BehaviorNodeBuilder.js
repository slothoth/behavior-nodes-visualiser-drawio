...
Draw.loadPlugin(function(ui) {
    ...
    function simplifyInterface() {
        // Remove sidebar
        const sidebarContainer = document.querySelector('.geSidebarContainer');
        if (sidebarContainer) {
            sidebarContainer.remove();
        }

        // remove splitter between graph and sidebar
        const hSplit = document.querySelector('.geHsplit');
        if (hSplit) {
            hSplit.remove();
        }

        // Adjust the graph container to take full width
        /*
        const graphContainer = document.querySelector('.geDiagramContainer');
        if (graphContainer) {
        ...
        /*
        const captureButton = ui.toolbar.addButton('insertIcon', 'Capture State', function() {
            ui.actions.get('captureStateAction').funct();
        });
    ...
    // Call simplifyInterface after a short delay to ensure DOM is ready
    setTimeout(simplifyInterface, 100);

    // Prevent sidebar from being created in the future
    ui.createSidebar = function() {
        console.log('Sidebar creation prevented');
        return null;
    };
