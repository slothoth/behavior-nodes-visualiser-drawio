/**
 * Database visualization plugin for diagrams.net with simplified toolbar
 */
Draw.loadPlugin(function(ui) {
    console.log('Plugin initialized');

    // Simplify the toolbar first
    function simplifyToolbar() {
        ...
        // Remove all existing items from the toolbar
        while (ui.toolbar.container.firstChild) {
            ui.toolbar.container.removeChild(ui.toolbar.container.firstChild);
        }

        // Add only the basic shapes we want
        const basicTools = [
            {name: 'square', tooltip: 'Square', icon: 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyNCIgaGVpZ2h0PSIyNCIgdmlld0JveD0iMCAwIDI0IDI0Ij48cmVjdCB4PSI0IiB5PSI0IiB3aWR0aD0iMTYiIGhlaWdodD0iMTYiIGZpbGw9Im5vbmUiIHN0cm9rZT0iY3VycmVudENvbG9yIiBzdHJva2Utd2lkdGg9IjIiLz48L3N2Zz4='},
            {name: 'circle', tooltip: 'Circle', icon: 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyNCIgaGVpZ2h0PSIyNCIgdmlld0JveD0iMCAwIDI0IDI0Ij48Y2lyY2xlIGN4PSIxMiIgY3k9IjEyIiByPSI4IiBmaWxsPSJub25lIiBzdHJva2U9ImN1cnJlbnRDb2xvciIgc3Ryb2tlLXdpZHRoPSIyIi8+PC9zdmc+'},
            {name: 'arrow', tooltip: 'Arrow', icon: 'data:image/svg+xml;base64,'}
        ];

        // Add the basic shape tools
        basicTools.forEach(tool => {
            const button = ui.toolbar.addButton('geSprite-' + tool.name, tool.tooltip, function() {
                let prototype;
                switch(tool.name) {
                    case 'square':
                        prototype = new mxCell('', new mxGeometry(0, 0, 120, 60), 'shape=rectangle');
                        break;
                    case 'circle':
                        prototype = new mxCell('', new mxGeometry(0, 0, 80, 80), 'shape=ellipse');
                        break;
                    case 'arrow':
                        ui.actions.get('connection').funct();
                        return;
                }

                if (prototype) {
                    prototype.vertex = true;
                    ui.dropAndConnect(null, [prototype], null, null, null, null);
                }
            });

            button.innerHTML = `<div class="geAdaptiveAsset" style="background-image: url('${tool.icon}'); background-position: center center; background-repeat: no-repeat; background-size: 18px; width: 22px; height: 20px;"></div>`;
        });

        // Add separator
        ui.toolbar.addSeparator();

        // Add the database operations buttons
        const loadDbButton = ui.toolbar.addButton('insertIcon', 'Load Database', function() {
            ui.actions.get('loadDatabase').funct();
        });
    ...
    // Call simplifyToolbar after initialization
    simplifyToolbar();

    // Rest of your existing plugin code here
